...
const allOrdersForGrandma = (req, res, next) => {
  ...
  db.any(
    `SELECT orders.id AS order_id, users.id AS user_id, dishes.id AS dish_id, name, dishes.type AS dish_type, dishes.user_id AS grandma_id, quantity, remaining_quantity FROM orders JOIN users ON users.id = orders.user_id FULL JOIN dishes ON dishes.id = orders.dish_id WHERE dishes.user_id =$1`,
    [grandma_id]
  )
    .then(order => {
      res.status(200).json({
