...
const filterByLabel = (req, res, next) => {
  ...
  db.any(
    "SELECT label_name, labels.id AS label_id, dishes.id AS dish_id, dishes.user_id AS grandma_id, longitude, latitude, first_name, last_name, timeframe FROM label_dishes JOIN dishes ON label_dishes.dish_id = dishes.id JOIN users ON dishes.user_id = users.id JOIN labels ON label_dishes.label_id = labels.id WHERE label_dishes.label_id = $1",
    [label_id]
  )
    .then(grandma => {
      res.status(200).json({
